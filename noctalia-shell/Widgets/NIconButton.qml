...
Item {
  ...
  Rectangle {
    ...
    color: root.enabled && root.hovering && !PanelService.openedPanel ? colorBgHover : colorBg
    radius: Math.min((customRadius >= 0 ? customRadius : Style.iRadiusL), width / 2)
    border.color: root.enabled && root.hovering && !PanelService.openedPanel ? colorBorderHover : colorBorder
