...
PanelWindow {
  ...
  color: {
    if (dimmerOpacity > 0 && isPanelOpen && !isPanelClosing) {
      // Don't show dimmer for SessionMenu in large buttons style
      var panel = PanelService.openedPanel;
      if (panel && panel.objectName === "sessionMenuPanel" && Settings.data.sessionMenu.largeButtonsStyle) {
        return "transparent";
      }
  ...
  BackgroundEffect.blurRegion: Settings.data.general.enableBlurBehind ? blurRegion : null
    Region {
      ...
      Region {
        x: (!barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? barPlaceholder.x : 0
        y: (!barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? barPlaceholder.y : 0
        width: (!barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? barPlaceholder.width : 0
        height: (!barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? barPlaceholder.height : 0
      ...
      Region {
        x: 0
        y: 0
        width: (barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? root.width : 0
        height: (barPlaceholder.isFramed && root.barShouldShow && !barPlaceholder.isHidden && Settings.data.bar.barBlurEnabled) ? root.height : 0
