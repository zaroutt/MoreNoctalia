...
Item {
  ...
  property color panelBackgroundColor: Qt.alpha(Color.mSurface, Style.effectivePanelOpacity)
  property bool showPanelOutline: true
  property color panelBorderColor: Color.mOutline
  property real panelOutlineWidth: Settings.data.ui.panelOutlineWidth ?? 0
  property bool panelOutlineEnabled: Settings.data.ui.panelOutlineEnabled ?? false
  ...
  // When true, content fades in linearly from the start (no 8% delay).
  // Use this for panels like SessionMenu where buttons should appear smoothly.
  property bool linearContentOpacity: false
  ...
  property bool openOpacityComplete: false // Tracks when opening opacity fade-in finishes
  ...
  function close() {
    ...
    // Keep panel opaque during close - size shrinks first, opacity fades when invisible
  ...
  function closeImmediately() {
    ...
    root.openOpacityComplete = false;
  ...
  // Opacity animation
  // Panel stays fully opaque (1.0) whenever it has size.
  // Opacity only goes to 0 when panel is fully invisible (after size reaches 0).
  opacity: {
    if (isPanelVisible)
      return 1.0; // Panel is fully opaque whenever it has size
    return 0.0;
  }
  ...
  // Timer to trigger opacity fade-in when size animation completes
  // Option A: size grows first, then opacity fades in
  Timer {
    id: opacityTrigger
    interval: Style.animationFast
    repeat: false
    onTriggered: {
      if (root.isPanelVisible && !root.sizeAnimationComplete) {
  ...
  Item {
    ...
    Item {
      id: panelBackground
      clip: true  // Prevent wallpaper from leaking through during animation
      ...
      readonly property real currentWidth: {
        // Use CACHED values during animation to prevent direction changes
        var animWidth = (isOpening || isClosing) ? root.cachedShouldAnimateWidth : shouldAnimateWidth;
        if (isClosing && opacityFadeComplete && animWidth)
          return 0;
        if (isClosing)
          return targetWidth;
        // Option A: size animation starts immediately on open
        if (isPanelVisible)
          return targetWidth;
        // If not animating width, start at target (no visual change)
        // If animating width, start at 0 (will animate to target)
        return animWidth ? 0 : targetWidth;
      }
      readonly property real currentHeight: {
        // Use CACHED values during animation to prevent direction changes
        var animHeight = (isOpening || isClosing) ? root.cachedShouldAnimateHeight : shouldAnimateHeight;
        if (isClosing && opacityFadeComplete && animHeight)
          return 0;
        if (isClosing)
          return targetHeight;
        // Option A: size animation starts immediately on open
        ...
        return animHeight ? 0 : targetHeight;
      }
      ...
      Behavior on width {
        ...
        NumberAnimation {
          ...
          // Premium timing: slightly faster for snappier feel
          // Use CACHED shouldAnimateWidth during animation
          duration: !panelBackground.dimensionsInitialized ? 0 : ((root.isOpening || root.isClosing) && !root.cachedShouldAnimateWidth) ? 0 : (root.isOpening || root.isClosing) ? Math.round(Style.animationFast * 0.9) : Style.animationNormal
          easing.type: Easing.BezierSpline
          // Spring-like easing: subtle overshoot (1.08) for organic feel
          easing.bezierCurve: root.isOpening ? [0.34, 1.15, 0.64, 1] : [0.55, 0.085, 0.68, 0.53]

          onRunningChanged: {
            // Safety: Zero-duration animation handling
            if (!running && duration === 0) {
              if (root.isClosing && panelBackground.width === 0 && root.cachedShouldAnimateWidth) {
            ...
            // When width shrink completes during close, finalize (use cached value)
            if (!running && root.isClosing && panelBackground.width === 0 && root.cachedShouldAnimateWidth) {
              Qt.callLater(root.finalizeClose);
            }
          }
        }
      ...
      Behavior on height {
        ...
        NumberAnimation {
          ...
          // During normal content resizing: always use normal duration
          // Premium timing: slightly faster for snappier feel
          // Use CACHED shouldAnimateHeight during animation
          duration: !panelBackground.dimensionsInitialized ? 0 : (root.isOpening && !root.cachedShouldAnimateHeight) ? 0 : root.isOpening ? Math.round(Style.animationNormal * 0.9) : (root.isClosing && !root.cachedShouldAnimateHeight) ? 0 : root.isClosing ? Math.round(Style.animationFast * 0.9) : Style.animationNormal
          easing.type: Easing.BezierSpline
          // Spring-like easing: subtle overshoot (1.08) for organic feel
          easing.bezierCurve: root.isOpening ? [0.34, 1.15, 0.64, 1] : [0.55, 0.085, 0.68, 0.53]
          ...
          onRunningChanged: {
            // Safety: Zero-duration animation handling
            if (!running && duration === 0) {
              if (root.isClosing && panelBackground.height === 0 && root.cachedShouldAnimateHeight) {
            ...
            // When height shrink completes during close, finalize (use cached value)
            if (!running && root.isClosing && panelBackground.height === 0 && root.cachedShouldAnimateHeight) {
              Qt.callLater(root.finalizeClose);
            }
          }
        }
      ...
      // Cached corner states — updated only when panel is fully open.
      // During open/close animations we reuse the cached values to avoid
      // flickering caused by geometry changes.
      property int _cachedTopLeft: 0
      property int _cachedTopRight: 0
      property int _cachedBottomLeft: 0
      property int _cachedBottomRight: 0

      function _updateCachedCorners() {
        _cachedTopLeft     = topLeftCornerState
        _cachedTopRight    = topRightCornerState
        _cachedBottomLeft  = bottomLeftCornerState
        _cachedBottomRight = bottomRightCornerState
      }

      // Smart corner state calculation based on bar attachment and edge touching
      // During transitions, return the cached value so the visual shape stays stable.
      property int topLeftCornerState: {
        if (isOpening || isClosing) return _cachedTopLeft;
        ...
        if (barTouchInverted || edgeInverted) {
          ...
          if (panelContent.touchingTopEdge)
            return 1; // Top edge: horizontal inversion
          if (panelContent.touchingTopBar)
            return 1; // Top bar: outer curve (normal)
          if (panelContent.touchingLeftBar)
            return 2; // Left bar: outer curve (normal)
          return -1; // Fallback: flat corner
        }
        return 0;
      }

      property int topRightCornerState: {
        if (isOpening || isClosing) return _cachedTopRight;
        // If bar is not visible, don't show outer corners based on bar attachment
        if (!root.barShouldShow) {
          // Only check edge touching, not bar touching
        ...
        if (barTouchInverted || edgeInverted) {
          ...
          if (panelContent.touchingTopBar)
            return 1; // Top bar: outer curve (normal)
          if (panelContent.touchingRightBar)
            return 2; // Right bar: outer curve (normal)
          return -1; // Fallback: flat corner
        }
        return 0;
      }
      ...
      property int bottomLeftCornerState: {
        if (isOpening || isClosing) return _cachedBottomLeft;
        // If bar is not visible, don't show outer corners based on bar attachment
        if (!root.barShouldShow) {
          // Only check edge touching, not bar touching
        ...
        if (barTouchInverted || edgeInverted) {
          ...
          if (panelContent.touchingBottomEdge)
            return 1; // Bottom edge: horizontal inversion
          if (panelContent.touchingBottomBar)
            return 1; // Bottom bar: outer curve (normal)
          if (panelContent.touchingLeftBar)
            return 2; // Left bar: outer curve (normal)
          return -1; // Fallback: flat corner
        }
      ...
      property int bottomRightCornerState: {
        if (isOpening || isClosing) return _cachedBottomRight;
        // If bar is not visible, don't show outer corners based on bar attachment
        if (!root.barShouldShow) {
          // Only check edge touching, not bar touching
    ...
    // Transparent background fallback — matches ShapePath area (edgeOvershoot: 4)
    // Panel background (uses Style.effectivePanelOpacity for transparency)
    Rectangle {
      id: panelBackgroundRect
      x: panelBackground.x
      y: panelBackground.y
      ...
      color: "transparent"
      visible: root.isPanelVisible || root.isClosing || root.isOpening
      opacity: root.isPanelVisible && !root.isClosing ? 1.0 : 0.0
      Behavior on opacity {
        enabled: root.isClosing || (!root.isPanelVisible && !root.isOpening)
        NumberAnimation {
    ...
    Loader {
      ...
      width: panelBackground.width
      height: panelBackground.height
      clip: true  // Prevent content from leaking outside panel bounds during animation
      sourceComponent: root.panelContent
      
      // Content always at full scale - no scale animation
      scale: 1.0

      // Content opacity: always 1.0 when visible (SmartPanel root opacity handles the fade)
      opacity: root.isPanelVisible ? 1.0 : 0.0
      ...
      onLoaded: {
        ...
        Qt.callLater(function () {
          ...
          // Update corner state cache BEFORE isPanelVisible becomes true.
          // At this point isOpening is still false, so corner states calculate
          // normally and we capture the correct snapped/outer-curve values.
          // This prevents the panel from showing inner-curve first then switching
          // to outer-curve during the open animation.
          var _ph = panelContent.geometryPlaceholder
          if (_ph && _ph._updateCachedCorners) {
            _ph._updateCachedCorners()
          }
          ...
          if (root.animationsDisabled) {
            ...
            // Option A: start timer to trigger opacity fade-in when size animation completes
