...
#                              output: /tmp/quick-config-annotate.png
...
case "$ACTION" in
  ...
  annotate-window)
    ...
    grim -g "$GEOM" /tmp/quick-config-annotate.png 2>/dev/null \
  ...
  pin)
    ...
    FILE="/tmp/quick-config-pin-$(date +%s%3N).png"
  ...
  palette)
    ...
    FILE="/tmp/quick-config-palette.png"
  ...
  qr)
    ...
    grim -g "$GEOMETRY" /tmp/quick-config-qr.png 2>/dev/null \
        || { echo "ERROR: qr: grim capture failed" >&2; exit 2; }

    zbarimg -q --raw /tmp/quick-config-qr.png 2>/dev/null
    ;;
