...
Variants {
    id: root
    property string imagePath: "/tmp/quick-config-annotate.png"
    ...
    delegate: PanelWindow {
        ...
        function _rebuildCache() {
            ...
            var pixUrl = "file:///tmp/quick-config-annotate-pixel.png?" + overlayWin._pixelCacheBust
        ...
        function requestZoom(scale) {
            ...
            if (scale === 1.0) {
                ...
                root.parseAndShow(region, "/tmp/quick-config-annotate.png", root._primaryScreen)
                _invalidateCache()
            ...
            zoomProc.exec({ command: [
                "bash", "-c",
                "magick /tmp/quick-config-annotate.png -resize "
                    + newW + "x" + newH + "! /tmp/quick-config-annotate-zoom.png 2>/dev/null"
            ]})
        }
        Process {
            id: pixelateProc
        ...
        function preparePixelImage() {
            var basePath = "/tmp/quick-config-annotate.png"
            ...
            pixelateProc.exec({ command: [
                "bash", "-c",
                "magick /tmp/quick-config-annotate.png -scale 5% -scale 2000% "
                    + "/tmp/quick-config-annotate-pixel.png 2>/dev/null"
            ]})
        }
        onPixelImgReadyChanged: {
            if (pixelImgReady) {
                var stale = "file:///tmp/quick-config-annotate-pixel.png?"
            ...
            onExited: (code) => {
                if (code === 0) {
                    root.parseAndShowZoomed(
                        root.lastRegion,
                        "/tmp/quick-config-annotate-zoom.png",
        ...
        Process {
            ...
            onExited: (code) => {
                ...
                overlayWin._doUpload("/tmp/quick-config-share.png")
            }
        }
        Process {
			id: uploadProc
        ...
        Connections {
            ...
            function onIsVisibleChanged() {
                ...
                if (!root.isVisible) {
                    ...
                    overlayWin.showSharePopover  = false
                    overlayWin.uploadFailed      = false
                    var pixUrl = "file:///tmp/quick-config-annotate-pixel.png?"
                        + overlayWin._pixelCacheBust
                    cacheCanvas.unloadImage(pixUrl)
                    drawCanvas.unloadImage(pixUrl)
                    drawCanvas.requestPaint()
                    cleanupProc.exec({ command: ["bash", "-c", "rm -f /tmp/quick-config-annotate-zoom.png"] })
                } else {
                ...
                delegate: Item {
                    ...
                    Image {
                        ...
                        source: overlayWin.pixelImgReady
                            ? "file:///tmp/quick-config-annotate-pixel.png?" + overlayWin._pixelCacheBust
                            : ""
                        fillMode: Image.Stretch
                        cache:    false
                        smooth:   false
        ...
        function flattenAndShare() {
            ...
            if (root.zoomScale > 1.0) {
                ...
                drawCanvas.grabToImage(function(result) {
                    if (!result || !result.saveToFile("/tmp/quick-config-overlay.png")) {
                        overlayWin.isUploading  = false
                        overlayWin.uploadFailed = true
                        return
                    }
                    flattenForShareProc.exec({ command: [
                        ...
                        "/tmp/quick-config-annotate.png",
                        "/tmp/quick-config-overlay.png"
                    ]})
                })
            }
        }
