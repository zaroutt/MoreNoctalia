...
ColumnLayout {
  ...
  property string valueCountColor: pluginApi?.pluginSettings?.countColor || pluginApi?.manifest?.metadata?.defaultSettings?.countColor || "none"
  ...
  NColorChoice {
    Layout.fillWidth: true
    label: pluginApi?.tr("settings.count_color.label")
    description: pluginApi?.tr("settings.count_color.desc")
    currentKey: root.valueCountColor
    onSelected: function(key) {
      root.valueCountColor = key;
    }
  }
  ...
  function saveSettings() {
    ...
    pluginApi.pluginSettings.countColor = root.valueCountColor;
